...
export default function CafeDetailModal({ cafe, onClose }: CafeDetailModalProps) {
  ...
  const seoImage = cafe.images?.hero || `https://shillongcafemap.in/cafe-photos/${slug}/hero-0.jpg`;
  const seoBreadcrumbs = [
    { name: "Home", url: "https://shillongcafemap.in/" },
    { name: "All Cafés", url: "https://shillongcafemap.in/?tab=cafes" },
    { name: cafe.neighborhood + " Cafés", url: `https://shillongcafemap.in/?tab=cafes&area=${cafe.neighborhood.toLowerCase().replace(/\s+/g, "-")}` },
    { name: cafe.name, url: cafeCanonical },
  ];
  const cafeSchema = buildCafeSchema(cafe);
  // ─────────────────────────────────────────────────────────────────────────
  ...
  return (
    <>
      {/* ── Per-café SEO injection ─────────────────────────────────────────
          Fires as soon as this modal mounts. SEO.tsx upserts <title>,
          <meta description>, <link canonical>, OG/Twitter tags, and
          a <script id="seo-jsonld"> with CafeOrCoffeeShop + BreadcrumbList.
          When the modal unmounts (onClose), the parent's tab-level <SEO>
          component re-mounts and restores the homepage/tab title.
      ──────────────────────────────────────────────────────────────────── */}
      <SEO
        title={seoTitle}
        description={seoDescription}
        canonical={cafeCanonical}
        image={seoImage}
